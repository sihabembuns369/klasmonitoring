      ...
      <div class="modal-dialog modal-lg ">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">NILAI UJIAN DIATAS > 59</h4>
            <button
              type="button"
              class="close"
              @click="close"
          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-md-12">
                <div class="header-presentase d-flex">
                  ...
                  <div
                    class="presentase-materi ml-auto pb-2 d-flex align-items-center"
                  >
                    <radial-progress-bar
                      class="rounded-circle radialbar-ujian mr-2"
                      :diameter="50"
                      :completed-steps="percentageExam"
                      :total-steps="100"
                      startColor="#34B53A"
                      stopColor="#34B53A"
                      :strokeWidth="5"
                    ...
                    <span class="font-weight-bold">Nilai ujian > 59</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="row chart-persentase-modal">
              <div class="col-md-12 border-top border-bottom mt-2 pt-2">
                <p class="text-muted">
                  Persentase nilai ujian pelajar setiap bulannya (akumulasi
                  penilaian per/id pelajar - per/nilai ujian > 59 - per/bulan)
                </p>
                <highcharts
                  style="height:250px"
...
export default {
  name: "ModalPresentaseNilaiUjian",
  components: {
    highcharts: Chart,
    RadialProgressBar,
  },
  ...
  methods: {
    ...
    async chartDetailExam() {
      let self = this;
      self.$Progress.start();
      let endPoint = `${process.env.VUE_APP_API_URL}/government/dashboard/percentage/exam/detail`;
        ...
        .then((res) => {
          if (res.data.data) {
            res.data.data.forEach((item) => {
              ...
              self.chartOptions.series[0].data.push(item.total_scored);
    ...
    async percentageExamAll() {
      let self = this;
      self.$Progress.start();
      let endPoint = `${process.env.VUE_APP_API_URL}/government/dashboard/percentage/exam`;

      self.percentageExam = 0;
      ...
      await broker.fetch
        ...
        .then((res) => {
          if (res.data.data) {
            ...
            self.percentageExam = parseFloat(percentageData);
          }
