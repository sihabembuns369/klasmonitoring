                    ...
                    <table class="table table-hover table-information ">
                      <thead class="bg-dark text-white">
                        <tr>
                          ...
                          <th
                            scope="col"
                            v-column-sortable:book_stock.total_book_stock
                            class="text-center"
                          >
                            Total buku
                          </th>
                          ...
                          <th
                            scope="col"
                            v-column-sortable:total_user_rent
                            class="text-center"
                          >
                            Peminjaman/tahun
                          </th>
                          <th scope="col" class="text-center">(%)persiswa</th>
                      ...
                      <tbody v-if="!searchLoading && libraryBook.length == 0">
                        <tr>
                          <td valign="top" colspan="10" class="text-center">
                      ...
                      <tbody v-else>
                        <tr
                          v-for="(item, index) in libraryBook"
                          :key="`${index}-libraryBook`"
                        >
                          <th scope="row">{{ index + 1 }}</th>
                          <td>{{ item.school_name }}</td>
                          <td class="text-center">
                            {{ item.book_stock.total_book_stock }}
                          </td>
                          <td class="text-center">{{ item.total_student }}</td>
                          <td class="text-center">
                            {{ item.book_stock.total_book_stock_rented }}
                          </td>
                          <td class="text-center">
                            {{ item.total_user_rent }} ({{
                              ((item.total_user_rent / item.total_student) *
                                100)
                                | formatNumberDecimal
                            }}%)
                          </td>
...
<script>
import ModalPerpustakaan from "../../../../components/Goverment/Modal/ModalPerpustakaan";
...
export default {
  name: "Perpustakaan",
  metaInfo: {
    title: "Monitoring Perpustakaan",
    titleTemplate: "%s | Pemerintah",
  },
  components: {
    "modal-detail-perpustakaan": ModalPerpustakaan,
  ...
  methods: {
    ...
    orderBy(sortFn) {
      ...
      this.libraryBook.sort(sortFn);
    },
    loadData(url = null) {
      let self = this;
      let endPoint = `${process.env.VUE_APP_API_URL}/government/library/school-library?limit=${self.limit}`;
    ...
    waitSearch() {
      let self = this;
      let endPoint = `${process.env.VUE_APP_API_URL}/government/library/book?limit=${self.limit}`;
